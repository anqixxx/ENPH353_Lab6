...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        ret,bin_img = cv2.threshold(gray,140,255,cv2.THRESH_BINARY_INV)         #Thresholding

        x_dim = bin_img.shape[0]
        y_dim = bin_img.shape[1]    
        line_start = 0
        line_end = 0

        for x in range(0, x_dim):         # Finds the location of the first white dot, which is where the line starts
            if (bin_img[-1][x] == 255):
                line_start = x
                break

        for x in range(line_start, x_dim):  # Finds the location of the first black dot after the white, which is where the line ends
            if (bin_img[-1][x] == 0):
                line_end = x
                break
        
        line_location = (int) (line_end+line_start)/2   # Find the center of the line

        if (line_location != 0):
            self.timeout = 0    # Resets the value of the timer if state != [0,0,0,0,0,0] 
            state[(int) (line_location*10/x_dim)] = 1 # Uses the weird way that Python rounds to determine where the state is

        else:
            self.timeout = self.timeout + 1
            if ( self.timeout > 2 ):           # If timer > 30 frames, done = True
                done = True
        
        cv2.circle(cv_image, (int(line_location), 10), 10, (255,0,0), 1, -1, 0)
        cv2.putText(cv_image, str(state), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
